...
TestData ExcelData = findTestData('BIFAST/DF001-NonBIFAST')
...
for (int i = 1; i <= ExcelData.getRowNumbers(); i++) {
	...
	Mobile.scrollToText('BANK JAGO TBK', FailureHandling.STOP_ON_FAILURE)
	
	Mobile.delay(5, FailureHandling.STOP_ON_FAILURE)
	
	Mobile.takeScreenshot('', FailureHandling.STOP_ON_FAILURE)
	
    Mobile.tap(findTestObject('TransferAntarBank/XCUIElementTypeStaticText - BANK JAGO TBK'), 5, FailureHandling.OPTIONAL)
    ...
    Mobile.tap(findTestObject('TransferAntarBank/XCUIElementTypeStaticText - BI Fast - Rp2.500'), 5, FailureHandling.OPTIONAL)
